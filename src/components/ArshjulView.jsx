...
/* Et ekte, syklisk arshjul for Styringsportalen.
   - Sirkulaer hjulvisning: 12 manedssektorer (Jan ovrest, med klokka).
   - Kategori-ringer (konsentriske spor) fra org.planCategories.
   - Hendelser tegnes som buer i sin ring, over en eller flere maneder.
   - "Na"-viser pa inneverende ar; arvelger; opprett/rediger/slett.
   - Hendelser uten ar = arlig rytme (vises hvert ar); med ar = arsspesifikke.
   - Lagres i aktiv portals JSONB som data.arshjulEvents (ingen migrasjon).
   Selvstendig komponent — eget tema, egne ikoner, ingen avhengigheter. */
...
const Btn = ({ children, onClick, variant = 'ghost', icon: Icon, style = {} }) => {
  const v = variant === 'brass' ? { bg: theme.brass, fg: '#fff', bd: theme.brass }
...
export default function ArshjulView({ data = {}, save = () => {}, currentUserId }) {
  ...
  // Kategorier/ringer fra portalens taksonomi (fallback til standard)
  ...
  const events = useMemo(() => allEvents
    ...
    .map((e) => ({ ...e, startMonth: Math.min(Math.max(e.startMonth || 1, 1), 12), endMonth: Math.min(Math.max(e.endMonth || e.startMonth || 1, 1), 12) }))
    .map((e) => ({ ...e, endMonth: Math.max(e.endMonth, e.startMonth) })), [allEvents, year]);

  const saveEvents = (next) => save({ ...data, arshjulEvents: next });
  const upsert = (ev) => {
    const list = allEvents.some((x) => x.id === ev.id) ? allEvents.map((x) => (x.id === ev.id ? ev : x)) : [...allEvents, ev];
    saveEvents(list); setEditing(null);
  };
    ...
    <div>
      <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', flexWrap: 'wrap', gap: 14, marginBottom: 20 }}>
        <div>
          ...
          <p style={{ fontSize: 13.5, color: theme.inkSoft, margin: '4px 0 0' }}>De faste rytmene gjennom året — møter, prosesser og milepæler, fordelt på måned og spor. Hendelser uten årstall gjentas hvert år.</p>
        </div>
      ...
      <div style={{ display: 'flex', gap: 24, flexWrap: 'wrap', alignItems: 'flex-start' }}>
        ...
        <div style={{ flex: '1 1 460px', minWidth: 320, maxWidth: 560 }}>
          <svg viewBox={`0 0 ${SIZE} ${SIZE}`} style={{ width: '100%', height: 'auto', display: 'block' }}>
            {/* ringspor (svakt tonet per kategori) */}
            {cats.map((c, i) => { const { r1, r2 } = ringR(i); return (
              <path key={'ring' + c.key} d={ann(r1 + 1, r2 - 1, 0, 1)} fill={c.color} opacity={0.07} />
            ); })}

            {/* manedssektorer (zebra) + klikkbar for ny hendelse */}
            {MND_KORT.map((_, i) => (
              <path key={'mz' + i} d={ann(R_LABEL_IN, R_LABEL_OUT, i / 12, (i + 1) / 12)}
                fill={i % 2 ? theme.surfaceAlt : theme.brassLight} stroke={theme.surface} strokeWidth="1"
                style={{ cursor: 'pointer' }} onClick={() => newEvent(i + 1)} />
            ))}
            {/* radielle skillelinjer */}
            ...
            {MND_KORT.map((m, i) => { const [x, y] = polar((R_LABEL_IN + R_LABEL_OUT) / 2, (i + 0.5) / 12); return (
              <text key={'ml' + i} x={x} y={y} textAnchor="middle" dominantBaseline="central"
                fontSize="12.5" fontWeight={focusMonth === i + 1 ? 700 : 600}
                fill={focusMonth === i + 1 ? theme.brass : theme.inkSoft} fontFamily="Manrope, sans-serif"
                style={{ cursor: 'pointer' }} onClick={() => setFocusMonth(focusMonth === i + 1 ? null : i + 1)}>{m}</text>
            ); })}
            ...
            {events.map((e) => {
              ...
              const col = (cats.find((c) => c.key === e.category) || {}).color || theme.brass;
              const on = hover === e.id;
              return (
                <path key={e.id} d={ann(r1 + 3, r2 - 3, f0, f1)} fill={col} opacity={on ? 1 : 0.82}
                  stroke={theme.surface} strokeWidth={on ? 2 : 1} style={{ cursor: 'pointer' }}
                  onMouseEnter={() => setHover(e.id)} onMouseLeave={() => setHover(null)} onClick={() => setEditing(e)} />
              );
            })}

            {/* na-viser */}
            {fNow >= 0 && (() => { const [x2, y2] = polar(R_LABEL_OUT, fNow); const [x1, y1] = polar(R_HUB - 2, fNow); return (
              <g key="now"><line x1={x1} y1={y1} x2={x2} y2={y2} stroke={theme.rust} strokeWidth="2" />
                <circle cx={x2} cy={y2} r="4" fill={theme.rust} /></g>
            ); })()}
            ...
            {hoverEv ? (
              <>
                <text x={C} y={C - 12} textAnchor="middle" fontSize="10.5" fontWeight="700" fill={theme.brass} fontFamily="Manrope, sans-serif">{(cats.find((c) => c.key === hoverEv.category) || {}).label}</text>
          ...
          {/* tegnforklaring (ringene) */}
          <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap', marginTop: 8, justifyContent: 'center' }}>
            {cats.map((c) => (
              <span key={c.key} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 12, color: theme.inkSoft }}>
        ...
        <div style={{ flex: '1 1 300px', minWidth: 280 }}>
          <div style={{ background: theme.surface, border: `1px solid ${theme.border}`, borderRadius: 12, overflow: 'hidden' }}>
            {MND_LANG.map((mnd, i) => {
              const m = i + 1;
              const evs = events.filter((e) => e.startMonth <= m && e.endMonth >= m);
              ...
              return (
                <div key={mnd} style={{ borderTop: i ? `1px solid ${theme.borderSoft}` : 'none', background: isFocus ? theme.brassLight : 'transparent' }}>
                  ...
                  {evs.map((e) => {
                    ...
                    const cont = e.startMonth < m; // fortsettelse fra forrige maned
                    return (
                      <div key={e.id} onClick={() => setEditing(e)} onMouseEnter={() => setHover(e.id)} onMouseLeave={() => setHover(null)}
                        style={{ display: 'flex', alignItems: 'center', gap: 9, padding: '7px 14px 7px 18px', cursor: 'pointer', opacity: cont ? 0.55 : 1 }}>
