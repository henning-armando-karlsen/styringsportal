          ...
          return (
            ...
            >
              <div style={{
                ...
                {m.initials || ((p) => p.length > 1 ? p.map(w=>w[0]).join('').toUpperCase() : m.name.slice(0,2).toUpperCase())(m.name.trim().split(/\s+/))}
              </div>
...
function PersonModal({ person, isAdmin, onClose, onSave }) {
  ...
  const handleSave = () => {
    ...
    const updated = { id: person.id, name: form.name.trim(), role: form.role.trim(), email: form.email.trim(), phone: form.phone.trim(),
      initials: ((p) => p.length > 1 ? p.map(w=>w[0]).join('').toUpperCase() : form.name.trim().slice(0,2).toUpperCase())(form.name.trim().split(/\s+/)) };
          ...
          <>
            <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 20 }}>
              <div style={{
                ...
                {person.initials || ((p) => p.length > 1 ? p.map(w=>w[0]).join('').toUpperCase() : person.name.slice(0,2).toUpperCase())(person.name.trim().split(/\s+/))}
              </div>
              <div>
